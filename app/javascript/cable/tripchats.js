...
if (msg) {
  createChannel({channel: 'TripchannelChannel', trip_id: msg.dataset.tripId},{
    received({content, catcher_username}) {
      msg.insertAdjacentHTML('beforeend', htmlChat(content, catcher_username))
      chat.value = ''
...
function htmlChat(content, catcherUsername) {
  ...
  const messageClass = parseInt(c.catcher_id) === parseInt(msg.dataset.catcherId) ? 'message-orange' : 'message-blue'
  return (`
